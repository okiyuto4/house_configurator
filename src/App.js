...
import TextureColorPicker from "./Components/TextureColorPicker";
...
function App() {
  // const [selectedModel, setSelectedModel] = useState("Rocket");
  ...
  const updateTextureColor = (textureColor, index) => {
    //controls.current.reset();
    if(textureColor == 'wall') {
      setTextureIndex(index);
    } else if(textureColor == 'color') {
      setColorIndex(index);
    }
  ...
  return (
    <>
      <TextureColorPicker updateTextureColor={updateTextureColor} />

      <Canvas shadows linear flat camera={{ position: [1, 0, 2] }}>
        <Environment environmentIntensity={0.25} files={['/env/px.png', '/env/nx.png', '/env/py.png', '/env/ny.png', '/env/pz.png', '/env/nz.png']}>
        </Environment>
        ...
        <OrbitControls ref={controls} maxDistance={5} minDistance={1.5} maxPolarAngle={1.5} minPolarAngle={1.5}/>
